...
#define RELAIPIN 3 // Anschluss des Lüfter-Relais (A6)
#define DHTPIN_1 8 // Datenleitung für den DHT-Sensor 1 (innen)
#define DHTPIN_2 9 // Datenleitung für den DHT-Sensor 2 (außen)
...
#define Korrektur_t_2  0.1    // Korrekturwert Außensensor Temperatur
#define Korrektur_h_1  0      // Korrekturwert Innensensor Luftfeuchtigkeit
#define Korrektur_h_2  0.7    // Korrekturwert Außensensor Luftfeuchtigkeit
//***********************************************************

#define SCHALTmin   4.0 // minimaler Taupunktunterschied, bei dem das Relais schaltet
#define HYSTERESE   1.0 // Abstand von Ein- und Ausschaltpunkt

#define TEMP1_min   4.0 // Minimale Innentemperatur, bei der die Lüftung aktiviert wird
#define TEMP2_min   4.0 // Minimale Außentemperatur, bei der die Lüftung aktiviert wird
...
//**** Taupunkte errechnen********
  ...
  if (rel == true)
  {
    ...
    lcd.print(("L" "\xF5" "fter AN"));
  } 
  else
  {                             
    digitalWrite(RELAIPIN, RELAIS_AUS); // Relais ausschalten
    lcd.print(("L" "\xF5" "fter AUS"));
  }

  // speichere neue zeit bei statusänderung
  if (rel != relOld)
  {
    relOld = rel;
    if (rel == true)
  ...
  // rechne zeit seit letzter änderung
  diffTime = millis() - lastRunTime;
  if(diffTime > 0)
  {
    unsigned long diffTimeMin = diffTime / 60000;
    lcd.setCursor(0,1);
    lcd.print(("Seit "));
    if(diffTimeMin < 60)
    { 
      lcd.print(diffTimeMin);
      lcd.print((" Minuten."));
    }
    else
    {
      lcd.print(diffTimeMin/60);
      lcd.print((":"));
      lcd.print(diffTimeMin % 60);
      lcd.print((" h"));
    }
  ...
  lcd.setCursor(0,2);
  lcd.print(("Z" "\xE1" "hler:"));
  lcd.print(counter);
  
  lcd.setCursor(0,3);
  lcd.print("DeltaTP1-2: ");
  lcd.print(DeltaTP);
  lcd.write((uint8_t)0);
  lcd.print(("C"));
